...
router = APIRouter()
@router.get("/catalog/", tags=["catalog"])
def get_armor_catalog():
    with db.engine.begin as connection:
        in_stock = connection.execute(sqlalchemy.text("""SELECT sku, name, type, modifier, price, COALESCE(SUM(change),0) AS total
                                                      FROM armor_inventory
                                                      JOIN armor_ledger ON armor_inventory.id = armor_id
                                                      GROUP BY sku, name, type, modifier, price
                                                      """))
...
def get_weapon_catalog():
    with db.engine.begin as connection:
        in_stock = connection.execute(sqlalchemy.text("""SELECT sku, name, type, modifier, price, COALESCE(SUM(change),0) AS total
                                                      FROM item_inventory
                                                      JOIN item_ledger ON item_inventory.id = item_id
                                                      GROUP BY sku, name, type, modifier, price
                                                      """))
